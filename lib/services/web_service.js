...
class WebService extends require('./service').Service {
  ...
  async setupWebApplication() {
    ...
    const secretKey = await Authentication.accessKey();
  ...
  async setupRoutes() {
    const controllerImplementationsDir = path.resolve(this.application.root(), './lib/controllers/implementations');
    ...
    _.map(controllerClasses, (Controller) => {
      _.map(Controller.routes(), (routeOptions) => {
        const route = new Route(Controller, routeOptions, this.application);
